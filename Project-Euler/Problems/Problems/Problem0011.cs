...
namespace Problems
{
    public class Problem0011 : IProblem<long>
    {
        private int[][] multiArray = new[]
            {
        ...
        public long Solve()
        {
            ...
            checked
            {
                ...
                for (var y = 0; y < multiArray.Length; y++)
                {
                    for (var x = 0; x < multiArray[0].Length; x++)
                    {
                        for (var direction = 0; direction < 4; direction++)
                        {
                            var numbers = readNumbers(x, y, direction, 4);
        ...
        private bool inArray(int[][] array, int x, int y)
        {
        ...
        private void incrementPosition(ref int x, ref int y, int direction)
        {
            switch (direction)
            {
                case 0:
        ...
        private IEnumerable<int> readNumbers(int originalX, int originalY, int direction, int count)
        {
            var x = originalX;
            var y = originalY;
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (inArray(multiArray, x, y))
                {
                    var number = multiArray[y][x];

                    numbers.Add(number);
                }

                incrementPosition(ref x, ref y, direction);
            }

            return numbers.Count == 4 ? numbers : Enumerable.Empty<int>();
        }
    }
}
